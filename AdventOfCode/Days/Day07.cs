...
public class Day07 : BaseDay
{
    ...
    public override ValueTask<string> Solve_1()
    {
        ...
        var cards = OrderCards(orderedHands.OrderBy(x => x.Item3).GroupBy(x => x.Item3).ToList(), cardPriorities);
    ...
    public override ValueTask<string> Solve_2()
    {
        ...
        foreach (var hand in _input)
        {
            var cardCounts = hand.Key.GroupBy(x => x).ToDictionary(x => x.First(), x => x.Count());

            if (cardCounts.TryGetValue('J', out int value) && cardCounts['J'] != 5)
            {
                var key = cardCounts.Where(x => x.Key != 'J').MaxBy(kvp => kvp.Value).Key;
                cardCounts[key] += value;
                cardCounts.Remove('J');
            }

            var handCount = string.Join(string.Empty, cardCounts.OrderByDescending(x => x.Value).Select(x => x.Value));
            orderedHands.Add((hand.Key, hand.Value, _handPriorities[handCount]));
        }
    ...
    private static List<int> OrderCards(List<IGrouping<int, (string, int, int)>> orderedHands, Dictionary<char, int> cardPriorities)
    {
        var cards = new List<int>();

        foreach (var item in orderedHands)
        {
